...
class ChordNode:
    ...
    def __init__(self, node_address: str, first_node=False):
        ...
        self.node_info.address_str = cu.ChordUtil.gen_address_str()
        self.node_info.node_id = cu.ChordUtil.hash_str_to_int(self.node_info.address_str)
    ...
    def join(self, node_address : str):
        ...
        tyukai_node = cu.ChordUtil.get_node_by_address(node_address)
        ...
        tantou_data_list : List[cu.KeyValue] = successor.get_copies_of_my_tantou_data(self.node_info.node_id, False)
        ...
        if self.node_info.predecessor_info != None:
            predecessor_node = cu.ChordUtil.get_node_by_address(cast(ni.NodeInfo, self.node_info.predecessor_info).address_str)
            predecessor_node.stabilize_successor()

        # 自ノードの情報、仲介ノードの情報、successorとして設定したノードの情報
        cu.ChordUtil.dprint("join," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_node(tyukai_node.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]))

    def global_put(self, data_id : int, value_str : str):
        target_node = self.search_node(data_id)
        target_node.put(data_id, value_str)
        cu.ChordUtil.dprint("global_put_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_node(target_node.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_data(data_id))

    def put(self, data_id : int, value_str : str):
        key_id_str = str(data_id)
        self.stored_data[key_id_str] = value_str
        cu.ChordUtil.dprint("put," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_data(data_id))

    # 得られた value の文字列を返す
    def global_get(self, data_id : int):
        cu.ChordUtil.dprint("global_get_0," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_data(data_id))
        ...
        if got_value_str == ChordNode.QUERIED_DATA_NOT_FOUND_STR:
            ...
            while tried_node_num < ChordNode.GLOBAL_GET_NEAR_NODES_TRY_MAX_NODES:
                if cur_predecessor.node_info.predecessor_info == None:
                    cu.ChordUtil.dprint("global_get_1,predecessor is None")
                    break
                cur_predecessor = cu.ChordUtil.get_node_by_address(cast(ni.NodeInfo,cur_predecessor.node_info.predecessor_info).address_str)
                got_value_str = cur_predecessor.get(data_id)
                tried_node_num += 1
                cu.ChordUtil.dprint("global_get_1," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                 + cu.ChordUtil.gen_debug_str_of_node(cur_predecessor.node_info) + ","
                                 + cu.ChordUtil.gen_debug_str_of_data(data_id) + ","
                                 + got_value_str + "," + str(tried_node_num))
                if got_value_str != ChordNode.QUERIED_DATA_NOT_FOUND_STR:
                    # データが円環上でIDが小さくなっていく方向（反時計時計回りの方向）を前方とした場合に
                    # 前方に位置するpredecessorを辿ることでデータを取得することができた
                    cu.ChordUtil.dprint("global_get_1_1," + "data found at predecessor,"
                                     + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                     + cu.ChordUtil.gen_debug_str_of_node(cur_predecessor.node_info))
                    break
                else:
                    # できなかった
                    cu.ChordUtil.dprint("global_get_1_1," + "data not found at predecessor,"
                                     + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                     + cu.ChordUtil.gen_debug_str_of_node(cur_predecessor.node_info))
                    break
        ...
        if got_value_str == ChordNode.QUERIED_DATA_NOT_FOUND_STR:
            ...
            while tried_node_num < ChordNode.GLOBAL_GET_NEAR_NODES_TRY_MAX_NODES:
                cur_successor = cu.ChordUtil.get_node_by_address(cast(ni.NodeInfo,cur_successor.node_info.successor_info_list[0]).address_str)
                got_value_str = cur_successor.get(data_id)
                tried_node_num += 1
                cu.ChordUtil.dprint("global_get_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                 + cu.ChordUtil.gen_debug_str_of_node(cur_successor.node_info) + ","
                                 + cu.ChordUtil.gen_debug_str_of_data(data_id) + ","
                                 + got_value_str + "," + str(tried_node_num))
                if got_value_str != ChordNode.QUERIED_DATA_NOT_FOUND_STR:
                    # データが円環上でIDが小さくなっていく方向（反時計時計回りの方向）を前方とした場合に
                    # 後方に位置するsuccessorを辿ることでデータを取得することができた
                    cu.ChordUtil.dprint("global_get_2_1," + "data found at successor,"
                                     + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                     + cu.ChordUtil.gen_debug_str_of_node(cur_successor.node_info))
                    break
                else:
                    # できなかった
                    cu.ChordUtil.dprint("global_get_2_1," + "data not found at successor,"
                                     + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                     + cu.ChordUtil.gen_debug_str_of_node(cur_successor.node_info))
                    break
        ...
        if ChordNode.need_getting_retry_data_id != -1:
            if got_value_str != ChordNode.QUERIED_DATA_NOT_FOUND_STR:
                # リトライに成功した
                cu.ChordUtil.dprint("global_get_2_5,retry success")
                # リトライは不要なためクリア
                ChordNode.need_getting_retry_data_id = -1
                ChordNode.need_getting_retry_node = None
            else:
                # リトライに失敗した（何もしない）
                cu.ChordUtil.dprint("global_get_2_5,retry failed")
                pass
        ...
        cu.ChordUtil.dprint("global_get_3," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
              + cu.ChordUtil.gen_debug_str_of_node(target_node.node_info) + ","
              + cu.ChordUtil.gen_debug_str_of_data(data_id) + "," + got_value_str)
    ...
    def get(self, data_id : int) -> str:
        ...
        cu.ChordUtil.dprint("get," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_data(data_id) + "," + ret_value_str)
        return ret_value_str

    # some_id をChordネットワーク上で担当するノードを返す
    def search_node(self, some_id : int) -> 'ChordNode':
        cu.ChordUtil.dprint("search_node_0," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_data(some_id))

        found_node = self.find_successor(some_id)
        if found_node == None:
            # TODO: ノード探索が失敗した場合は、一定時間を空けてリトライするようにする
            cu.ChordUtil.dprint("search_node_1," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                             + cu.ChordUtil.gen_debug_str_of_data(some_id))
            raise Exception("appropriate node was not found.")

        found_node = cast(ChordNode, found_node)
        cu.ChordUtil.dprint("search_node_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_node(found_node.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_data(some_id))
    ...
    def get_copies_of_my_tantou_data(self, node_id : int, rest_copy : bool = True) -> List[cu.KeyValue]:
        ret_datas : List[cu.KeyValue] = []
        for key, value in self.stored_data.items():
            ...
            # 除外する
            if cu.ChordUtil.exist_between_two_nodes_right_mawari(node_id, self.node_info.node_id, data_id):
                continue

            # 文字列の参照をそのまま用いてしまうが、文字列はイミュータブルであるため
            # 問題ない
            item = cu.KeyValue(None, value)
    ...
    def check_predecessor(self, id : int, node_info : ni.NodeInfo):
        if self.node_info.predecessor_info == None:
            # 未設定状態なので確認するまでもなく、predecessorらしいと判断し
            # 経路情報に設定し、処理を終了する
            self.node_info.predecessor_info = node_info.get_partial_deepcopy()
            cu.ChordUtil.dprint("check_predecessor_1," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                  + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]) + ","
                  + cu.ChordUtil.gen_debug_str_of_node(self.node_info.predecessor_info))

            return

        cu.ChordUtil.dprint("check_predecessor_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
              + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]))

        distance_check = cu.ChordUtil.calc_distance_between_nodes_left_mawari(self.node_info.node_id, id)
        distance_cur = cu.ChordUtil.calc_distance_between_nodes_left_mawari(self.node_info.node_id, cast(ni.NodeInfo,self.node_info.predecessor_info).node_id)
        ...
        if distance_check < distance_cur:
            ...
            cu.ChordUtil.dprint("check_predecessor_3," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                  + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]) + ","
                  + cu.ChordUtil.gen_debug_str_of_node(self.node_info.predecessor_info))
    ...
    def stabilize_successor(self):
        cu.ChordUtil.dprint("stabilize_successor_1," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
              + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]))
        ...
        if self.node_info.predecessor_info != None and (self.node_info.node_id == self.node_info.successor_info_list[0].node_id):
            cu.ChordUtil.dprint("stabilize_successor_1_5," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                             + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]))
        ...
        # の参照を得る
        successor = cu.ChordUtil.get_node_by_address(self.node_info.successor_info_list[0].address_str)
        ...
        if successor_info.predecessor_info == None:
            ...
            cu.ChordUtil.dprint("stabilize_successor_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                             + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]))
            return

        cu.ChordUtil.dprint("stabilize_successor_3," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]))

        pred_id_of_successor = successor_info.predecessor_info.node_id

        cu.ChordUtil.dprint("stabilize_successor_3_5," + hex(pred_id_of_successor))
        ...
        if(pred_id_of_successor == self.node_info.node_id):
            ...
            successor_obj = cu.ChordUtil.get_node_by_address(successor_info.address_str)
            successor_obj.check_predecessor(self.node_info.node_id, self.node_info)

            distance_unknown = cu.ChordUtil.calc_distance_between_nodes_left_mawari(successor_obj.node_info.node_id, pred_id_of_successor)
            distance_me = cu.ChordUtil.calc_distance_between_nodes_left_mawari(successor_obj.node_info.node_id, self.node_info.node_id)
            if distance_unknown < distance_me:
                ...
                # ば情報を更新してもらう
                new_successor_obj = cu.ChordUtil.get_node_by_address(self.node_info.successor_info_list[0].address_str)
                new_successor_obj.check_predecessor(self.node_info.node_id, self.node_info)

                cu.ChordUtil.dprint("stabilize_successor_4," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                 + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]) + ","
                                 + cu.ChordUtil.gen_debug_str_of_node(new_successor_obj.node_info))

    # FingerTableに関するstabilize処理を行う
    # 一回の呼び出しで1エントリを更新する
    # FingerTableのエントリはこの呼び出しによって埋まっていく
    def stabilize_finger_table(self, idx):
        cu.ChordUtil.dprint("stabilize_finger_table_1," + cu.ChordUtil.gen_debug_str_of_node(self.node_info))

        # FingerTableの各要素はインデックスを idx とすると 2^IDX 先のIDを担当する、もしくは
        # 担当するノードに最も近いノードが格納される
        update_id = cu.ChordUtil.overflow_check_and_conv(self.node_info.node_id + 2**idx)
        found_node = self.find_successor(update_id)
        # found_node = self.find_predecessor(update_id)
        if found_node == None:
            cu.ChordUtil.dprint("stabilize_finger_table_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info))
            return

        self.node_info.finger_table[idx] = found_node.node_info.get_partial_deepcopy()

        cu.ChordUtil.dprint("stabilize_finger_table_3," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_node(found_node.node_info))

    # id（int）で識別されるデータを担当するノードの名前解決を行う
    def find_successor(self, id : int) -> Optional['ChordNode']:
        cu.ChordUtil.dprint("find_successor_1," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
              + cu.ChordUtil.gen_debug_str_of_data(id))

        n_dash = self.find_predecessor(id)
        if n_dash == None:
            cu.ChordUtil.dprint("find_successor_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                             + cu.ChordUtil.gen_debug_str_of_data(id))
            return None

        cu.ChordUtil.dprint("find_successor_3," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_node(n_dash.node_info) + ","
                         + cu.ChordUtil.gen_debug_str_of_node(self.node_info.successor_info_list[0]) + ","
                         + cu.ChordUtil.gen_debug_str_of_data(id))

        return cu.ChordUtil.get_node_by_address(n_dash.node_info.successor_info_list[0].address_str)

    # id(int)　の前で一番近い位置に存在するノードを探索する
    def find_predecessor(self, id: int) -> 'ChordNode':
        cu.ChordUtil.dprint("find_predecessor_1," + cu.ChordUtil.gen_debug_str_of_node(self.node_info))
        ...
        while not cu.ChordUtil.exist_between_two_nodes_right_mawari(cast(ni.NodeInfo,n_dash.node_info).node_id, cast(ni.NodeInfo, n_dash.node_info.successor_info_list[0]).node_id, id):
            cu.ChordUtil.dprint("find_predecessor_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                             + cu.ChordUtil.gen_debug_str_of_node(n_dash.node_info))
            ...
            if n_dash_found.node_info.node_id == n_dash.node_info.node_id:
                ...
                cu.ChordUtil.dprint("find_predecessor_3," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                 + cu.ChordUtil.gen_debug_str_of_node(n_dash.node_info))
                return n_dash_found

            # closelst_preceding_finger は id を通り越してしまったノードは返さない
            # という前提の元で以下のチェックを行う
            distance_old = cu.ChordUtil.calc_distance_between_nodes_right_mawari(self.node_info.node_id, n_dash.node_info.node_id)
            distance_found = cu.ChordUtil.calc_distance_between_nodes_right_mawari(self.node_info.node_id, n_dash_found.node_info.node_id)
            distance_data_id = cu.ChordUtil.calc_distance_between_nodes_right_mawari(self.node_info.node_id, id)
            if distance_found < distance_old and not (distance_old >= distance_data_id):
                ...
                cu.ChordUtil.dprint("find_predecessor_4," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                 + cu.ChordUtil.gen_debug_str_of_node(n_dash.node_info))

                return n_dash

            cu.ChordUtil.dprint("find_predecessor_5_n_dash_updated," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                             + cu.ChordUtil.gen_debug_str_of_node(n_dash.node_info) + "->"
                             + cu.ChordUtil.gen_debug_str_of_node(n_dash_found.node_info))
    ...
    def closest_preceding_finger(self, id : int) -> 'ChordNode':
        ...
        for entry in reversed(self.node_info.finger_table):
            # 埋まっていないエントリも存在し得る
            if entry == None:
                cu.ChordUtil.dprint("closest_preceding_finger_0," + cu.ChordUtil.gen_debug_str_of_node(self.node_info))
                continue

            cu.ChordUtil.dprint("closest_preceding_finger_1," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                  + cu.ChordUtil.gen_debug_str_of_node(entry))
            ...
            if cu.ChordUtil.exist_between_two_nodes_right_mawari(self.node_info.node_id, id, entry.node_id):
                cu.ChordUtil.dprint("closest_preceding_finger_2," + cu.ChordUtil.gen_debug_str_of_node(self.node_info) + ","
                                 + cu.ChordUtil.gen_debug_str_of_node(entry))
                return cu.ChordUtil.get_node_by_address(entry.address_str)

        cu.ChordUtil.dprint("closest_preceding_finger_3")
