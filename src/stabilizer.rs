...
pub fn set_routing_infos_force(self_node: ArMu<node_info::NodeInfo>, client_pool: ArMu<HashMap<String, ArMu<reqwest::blocking::Client>>>, predecessor_info: node_info::NodeInfo, successor_info_0: node_info::NodeInfo , ftable_enry_0: node_info::NodeInfo){
...
pub fn join(new_node: ArMu<node_info::NodeInfo>, client_pool: ArMu<HashMap<String, ArMu<reqwest::blocking::Client>>>, self_node_address: &String, tyukai_node_address: &String, born_id: i32){
    ...
    let successor = endpoints::rrpc_call__find_successor(&tyukai_node, Arc::clone(&client_pool), deep_cloned_new_node.node_id).unwrap();
    ...
    match endpoints::rrpc_call__check_predecessor(&successor, &deep_cloned_new_node, Arc::clone(&client_pool)){
        Err(err) => {
            ...
            join(new_node, Arc::clone(&client_pool), self_node_address, tyukai_node_address, born_id);
        }
        Ok(some) => {}
    };
}
...
// する際に利用することを想定する
pub fn pass_datas(self_node: ArMu<node_info::NodeInfo>, data_store: ArMu<data_store::DataStore>, client_pool: ArMu<HashMap<String, ArMu<reqwest::blocking::Client>>>, pass_datas: Vec<chord_util::DataIdAndValue>) -> Result<bool, chord_util::GeneralError> {
    let mut data_store_ref = data_store.lock().unwrap();
